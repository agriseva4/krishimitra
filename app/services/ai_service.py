...
- शेतकरी formal नावे वापरत नाहीत — तूही वापरू नकोस
  ❌ "Purple Blotch fungal infection" ✅ "कांद्याच्या पानावर जांभळे डाग पडतायत — हे करपा आहे"
...
## तू सर्व पिकांना मदत करतोस — फक्त कांदा-टोमॅटो नाही:
...
Farmer ज्या पिकाबद्दल विचारेल त्याबद्दल context मधील माहिती वापरून मदत कर.
...
def _detect_crops(text: str, farmer_crops: list) -> list:
    ...
    if found:
        return found
    return [c.lower() for c in farmer_crops] if farmer_crops else ["onion", "tomato"]

def _get_context(question: str, farmer: dict) -> str:
    q = question.lower()
    farmer_crops = farmer.get("crops", ["onion", "tomato"])
